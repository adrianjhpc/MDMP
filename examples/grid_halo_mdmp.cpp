  ...
  for(int d=0;d<Nd;d++){
    if ( cart_geom[d]>1 ) {
      ...
      for(int n=0;n<ncall;n++){
        ...
        // --- Forward Shift (Asynchronous MDMP Region) ---
        MDMP_COMMREGION_BEGIN();
        MDMP_SEND((char*)xmit, bytes, rank, to, rank);
        MDMP_RECV((char*)recv, bytes, rank, from, from);
        MDMP_COMMREGION_END();
        ...
        // --- Backward Shift (Asynchronous MDMP Region) ---
        MDMP_COMMREGION_BEGIN();
        MDMP_SEND((char*)xmit, bytes, rank, to, rank);
        MDMP_RECV((char*)recv, bytes, rank, from, from);
        MDMP_COMMREGION_END();
