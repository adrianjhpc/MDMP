...
int main(int argc, char** argv) {
    ...
    for (int iter = 0; iter < cfg.iters; ++iter) {
        ...
        // Register receives first
        if (P.have_left) {
            MDMP_REGISTER_RECV(P.recv_left.data(),
                               P.left_count,
                               rank,
                               P.left_rank,
        ...
        if (P.have_right) {
            MDMP_REGISTER_RECV(P.recv_right.data(),
                               P.right_count,
                               rank,
                               P.right_rank,
        ...
        if (P.have_left) {
            MDMP_REGISTER_SEND(P.send_left.data(),
                               P.left_count,
                               rank,
                               P.left_rank,
                               SpMVProblem::TAG_SEND_FIRST);
        ...
        if (P.have_right) {
            MDMP_REGISTER_SEND(P.send_right.data(),
                               P.right_count,
                               rank,
                               P.right_rank,
                               SpMVProblem::TAG_SEND_LAST);
    ...
    if (rank == 0) {
        std::cout << "SpMV MDMP Declarative benchmark on " << size << " ranks\n";
