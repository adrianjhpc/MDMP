...
/*
  ...
  Purpose:
    ...
    MAIN is the main program for HEAT_MDMP_REGCOM.
*/
  {
  int id;
  int p;
  ...
  if ( id == 0 )
  {
    ...
    printf ( "\n" );
    printf ( "HEAT_MDMP:\n" );
    printf ( "  C/MDMP Declarative Inspector-Executor version\n" );
...
*/
  for ( j = 1; j <= j_max; j++ )
  {
    ...
    MDMP_REGISTER_SEND( &h[1],   1, id, id - 1, 1 ); 
    MDMP_REGISTER_RECV( &h[n+1], 1, id, id + 1, 1 );

    // Tag 2: Rightward Send, Leftward Recv
    MDMP_REGISTER_SEND( &h[n],   1, id, id + 1, 2 );
    MDMP_REGISTER_RECV( &h[0],   1, id, id - 1, 2 );
    ...
    // Compute the boundary points (These rely on h[0] and h[n+1])
    // Left boundary
    h_new[1] = h[1] 
...
*/
    time = time_new;
    // time = time_new; // Removed duplicated assignment from original code
