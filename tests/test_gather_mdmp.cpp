...
int main() {
    ...
    if (rank == 0) {
        ...
        std::cout << "=== MDMP Asynchronous Gather Test ===" << std::endl;
        std::cout << "World Size: " << size << " | Items per rank: " << send_count << "\n" << std::endl;
    }

    // The MDMP Communication Region
    MDMP_COMMREGION_BEGIN();

    // Every rank contributes 'send_count' items. 
    // Only Rank 0 (the root) needs a valid recv_buf.
    MDMP_GATHER(send_buf, send_count, recv_buf, 0);
    ...
    if (rank == 0) {
        ...
        if (passed) {
            std::cout << "[PASS] Root successfully gathered and stitched the global array!" << std::endl;
        } else {
